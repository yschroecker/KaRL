...
class LikelihoodRatioGradient:
    def __init__(self, policy, critic, optimizer):
        ...
        advantages = critic.advantages(states, actions, next_states, rewards, is_terminal, policy)
    ...
    def _policy_gradient(self, states, actions, advantages, is_terminal):
        ...
        return T.grad(-T.sum(log_action_probabilities * advantages, axis=0),
...
class TDVAdvantageEstimator:
    def __init__(self, value_learner, discount_factor):
        self._td_learner = value_learner
        self._discount_factor = discount_factor

    def advantages(self, states, actions, next_states, rewards, is_terminal, policy):
        values = self._td_learner.value(states)
        next_values = self._td_learner.value(next_states)  # TODO: reuse values from state
        return rewards + is_terminal * self._discount_factor * next_values - values
